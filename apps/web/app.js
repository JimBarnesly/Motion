...
async function commit(type,payload,localMutation) {
  ...
  try {
    if (adapter.kind === "tauri") { const result=await adapter.execute(type,payload); state.workspace=canonicalWorkspace(result.workspace); state.revision=result.revision; }
...
async function confirmCanonicalEdit({candidate}) {
  if(adapter.kind==="tauri"){
    ...
    state.workspace=canonicalWorkspace(result.workspace);state.revision=result.revision;
  }else{
...
async function ensureWorkspace() {
  if (workspace()) return;
  if (adapter.kind === "tauri") { const result=await adapter.execute("workspace.create",{name:"Motion Workspace"}); state.workspace=canonicalWorkspace(result.workspace); state.revision=result.revision; }
...
document.addEventListener("click",async event=>{const button=event.target.closest("button");if(!button)return;try{
  ...
  if(button.dataset.moveBlock){checkpoint();const[id,delta]=delegatedIdAndDelta(button.dataset.moveBlock),at=page.blocks.findIndex(block=>block.id===id),to=at+delta;if(to>=0&&to<page.blocks.length)[page.blocks[at],page.blocks[to]]=[page.blocks[to],page.blocks[at]];await replaceBlocks(page);return render();}
