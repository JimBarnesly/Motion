...
const attachmentIngestion=createAttachmentIngestion({activePage,ingest:payload=>adapter.ingestAttachmentBlock(payload),digest:sha256,confirm:result=>{confirmNativeSnapshot(result.workspace,result.revision);render();},status:message=>{$("#saveState").textContent=message;}});
...
function renderBlocks(page){return (page.blocks??[]).map((block,index)=>{const type=BLOCK_TYPES.includes(block.type)?block.type:"unsupported";return `<div class="block-row ${type}" data-block-id="${escapeAttribute(block.id)}"><div class="block-tools"><button data-move-block="${escapeAttribute(block.id)}:-1" ${index?"":"disabled"}>↑</button><button data-move-block="${escapeAttribute(block.id)}:1" ${index<page.blocks.length-1?"":"disabled"}>↓</button></div><select data-block-type="${escapeAttribute(block.id)}" aria-label="Block type">${renderBlockTypeOption(block.type,BLOCK_LABELS[block.type]??"Unsupported")}${BLOCK_TYPES.filter(item=>item!==block.type).map(item=>`<option value="${item}">${BLOCK_LABELS[item]}</option>`).join("")}</select>${block.type==="task"?`<input class="task-check" type="checkbox" data-task="${escapeAttribute(block.id)}" ${block.checked?"checked":""}>`:""}${block.type==="divider"?`<hr>`:`<div class="block-input ${type}" contenteditable="true" data-block="${escapeAttribute(block.id)}" data-placeholder="Type text, or [[Page name]]">${escapeHtml(block.text)}</div>`}<button class="delete-block" data-delete-block="${escapeAttribute(block.id)}">×</button></div>`;}).join("");}
