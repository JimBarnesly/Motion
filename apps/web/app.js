...
function renderBlocks(page){return (page.blocks??[]).map((block,index)=>{const type=BLOCK_TYPES.includes(block.type)?block.type:"unsupported",attachment=block.type==="file"?workspace().attachments.find(item=>item.id===block.attachmentId):null,fileContent=block.type==="file"?(attachment?`<div class="block-input file" role="group" aria-label="Attachment ${escapeHtml(attachment.fileName)}"><strong>📎 ${escapeHtml(attachment.fileName)}</strong><small>${escapeHtml(attachment.mediaType)} · ${attachment.byteLength} bytes</small><button data-download-attachment="${escapeAttribute(attachment.id)}">Download</button></div>`:`<div class="block-input file unavailable" role="alert">Attachment unavailable: metadata is missing or corrupt.</div>`):null;return `<div class="block-row ${type}" data-block-id="${escapeAttribute(block.id)}"><div class="block-tools"><button data-move-block="${escapeAttribute(block.id)}:-1" ${index?"":"disabled"}>↑</button><button data-move-block="${escapeAttribute(block.id)}:1" ${index<page.blocks.length-1?"":"disabled"}>↓</button></div>${block.type==="file"?`<span class="block-type-label">File</span>`:`<select data-block-type="${escapeAttribute(block.id)}" aria-label="Block type">${renderBlockTypeOption(block.type,BLOCK_LABELS[block.type]??"Unsupported")}${BLOCK_TYPES.filter(item=>item!==block.type).map(item=>`<option value="${item}">${BLOCK_LABELS[item]}</option>`).join("")}</select>`}${block.type==="task"?`<input class="task-check" type="checkbox" data-task="${escapeAttribute(block.id)}" ${block.checked?"checked":""}>`:""}${fileContent??(block.type==="divider"?`<hr>`:`<div class="block-input ${type}" contenteditable="true" data-block="${escapeAttribute(block.id)}" data-placeholder="Type text, or [[Page name]]">${escapeHtml(block.text)}</div>`)}<button class="delete-block" data-delete-block="${escapeAttribute(block.id)}">×</button></div>`;}).join("");}
