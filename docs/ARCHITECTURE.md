...
- **Link index:** materialized incoming/outgoing page and block references derived from canonical block payloads. Content and index rows update in one transaction; the index is rebuildable and consistency-checkable.
...
- Unknown block types remain canonical opaque records: their type discriminator, version, and complete payload pass through edits, sync, and exports unchanged while the UI shows an unsupported-block placeholder.
