...
| Restore and record integrity | Integrated and compiled locally | Public verification cross-checks attachment payload ownership/metadata and record membership; schema-directed restoration preserves opaque/external fields; all public model mutators preserve membership; focused Backup/Core suites pass. |
...
| Focused package suites | Partially verified | Core 31/31, Backup 9/9, and Web 54/54 passed. App-service runtime is blocked under Node 20 because `node:sqlite` requires Node 22. |
