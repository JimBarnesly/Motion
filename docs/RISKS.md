...
|---|---|---|
| Native Linux packages are built but their installed UI is not smoke-tested | Open, high | CI run `30876348219` built and uploaded x86-64 and ARM64 package artifacts. The AppImage gate extracts the bundled runtime and proves offline save, termination, restart, reload, search, backup, and restore; install each artifact on a representative host and prove native UI launch and interaction. |
...
| Packaged native UI restart and crash-boundary recovery remain unverified | Open, high | Source-tree and extracted-AppImage service restart tests pass with networking denied. Add native UI restart plus concurrency, disk-full, and forced-termination tests around transaction boundaries. |
...
| Large attachments/imports can exhaust memory or resources | Open, high | Attachments are currently fully buffered across browser and IPC boundaries; implement streaming, quotas, canonical paths, archive limits, and sanitisation tests. |
