...
export function assertWorkspaceValue(value: unknown, overrides: Partial<ValidationLimits> = {}): asserts value is Workspace {
  ...
  for (const [index, item] of w.databases.entries()) {
    ...
    if (new Set(orderedPropertyIds).size !== orderedPropertyIds.length || orderedPropertyIds.length !== liveProperties.length || orderedPropertyIds.some(propertyId => properties.get(propertyId)?.deletedAt !== undefined)) fail(`${path}.propertyOrder must contain every live property exactly once`);
