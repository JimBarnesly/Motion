...
export function assertWorkspaceValue(value: unknown, overrides: Partial<ValidationLimits> = {}): asserts value is Workspace {
  ...
  for (const [index, item] of w.databases.entries()) {
    ...
    if (db.properties.some(property => property.type === "title") && !liveProperties.some(property => property.type === "title")) fail(`${path} must retain a live title property`);
    if (liveProperties.some(property => property.type === "title") && !orderedPropertyIds.some(propertyId => livePropertyMap.get(propertyId)?.type === "title")) fail(`${path}.propertyOrder must retain the title property`);
