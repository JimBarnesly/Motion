...
function validatePropertyValue(value: unknown, property: DatabaseProperty, path: string, limits: ValidationLimits, attachmentIds: ReadonlySet<ID>): void {
  ...
  if (validation !== undefined) {
    if (typeof value === "number") {
      if (validation.min !== undefined && value < validation.min) fail(`${path} is below validation minimum`);
      if (validation.max !== undefined && value > validation.max) fail(`${path} is above validation maximum`);
    }
    if (typeof value === "string") {
      if (validation.minLength !== undefined && value.length < validation.minLength) fail(`${path} is below validation minimum length`);
      if (validation.maxLength !== undefined && value.length > validation.maxLength) fail(`${path} exceeds validation maximum length`);

    }
  }
}

function validatePropertyMetadata(property: DatabaseProperty, path: string, limits: ValidationLimits): void {
  if (property.deletedAt !== undefined) timestamp(property.deletedAt, `${path}.deletedAt`, limits);
  if (property.validation === undefined) return;
  if (!plain(property.validation) || Object.keys(property.validation).some(key => !["required", "min", "max", "minLength", "maxLength"].includes(key))) fail(`${path}.validation has an invalid shape`);
  const validation = property.validation as PropertyValidation;
  if (validation.required !== undefined && typeof validation.required !== "boolean") fail(`${path}.validation.required must be a boolean`);
  for (const key of ["min", "max"] as const) { const bound = validation[key]; if (bound !== undefined && (typeof bound !== "number" || !Number.isFinite(bound))) fail(`${path}.validation.${key} must be finite`); }
  for (const key of ["minLength", "maxLength"] as const) { const bound = validation[key]; if (bound !== undefined && (typeof bound !== "number" || !Number.isSafeInteger(bound) || bound < 0)) fail(`${path}.validation.${key} must be a non-negative integer`); }
  if (typeof validation.min === "number" && typeof validation.max === "number" && validation.min > validation.max) fail(`${path}.validation min/max must be ordered`);
  if (typeof validation.minLength === "number" && typeof validation.maxLength === "number" && validation.minLength > validation.maxLength) fail(`${path}.validation length bounds must be ordered`);
...
export function assertWorkspaceValue(value: unknown, overrides: Partial<ValidationLimits> = {}): asserts value is Workspace {
  ...
  const pageIds = new Set<ID>(), pageById = new Map<ID, Page>(), databaseIds = new Set<ID>(), viewIds = new Set<ID>(), attachmentIds = new Set<ID>(), allIds = new Set<ID>([w.id]), blockOwners = new Map<ID, ID>();
  ...
  for (const [index, item] of w.pages.entries()) {
    ...
    const id = unique(page.id, `${path}.id`, limits, allIds); pageIds.add(id); pageById.set(id, page);
  ...
  let rows = 0; const recordPageOwners = new Map<ID, ID>();
  for (const [index, item] of w.databases.entries()) {
    ...
    const properties = new Map<ID, DatabaseProperty>(); for (const [p, prop] of db.properties.entries()) { const here = `${path}.properties[${p}]`; if (!plain(prop)) fail(`${here} must be a plain object`); if (Object.keys(prop).some(key => !["id", "name", "type", "relation", "relationDatabaseId", "options", "validation", "deletedAt"].includes(key))) fail(`${here} property has an invalid shape`); const propId = unique(prop.id, `${here}.id`, limits, allIds); properties.set(propId, prop as DatabaseProperty); string(prop.name, `${here}.name`, limits, true); oneOf(prop.type, PROPERTY_TYPES, `${here}.type`, limits); validatePropertyMetadata(prop, here, limits); if (prop.options !== undefined) { if (!Array.isArray(prop.options)) fail(`${here}.options must be an array`); for (const [o, option] of prop.options.entries()) { if (!plain(option) || Object.keys(option).some(key => !["id", "name", "color"].includes(key))) fail(`${here}.options[${o}] must be a closed plain object`); unique(option.id, `${here}.options[${o}].id`, limits, allIds); string(option.name, `${here}.options[${o}].name`, limits, true); if (option.color !== undefined) string(option.color, `${here}.options[${o}].color`, limits); } } if (prop.relation !== undefined) { if (!plain(prop.relation) || Object.keys(prop.relation).some(key => !["targetCollectionId", "reciprocalPropertyId", "cardinality", "maxItems", "onDelete"].includes(key))) fail(`${here}.relation must be a closed plain object`); stableId(prop.relation.targetCollectionId, `${here}.relation.targetCollectionId`, limits); if (prop.relation.reciprocalPropertyId !== undefined) stableId(prop.relation.reciprocalPropertyId, `${here}.relation.reciprocalPropertyId`, limits); if (prop.relation.cardinality !== undefined) oneOf(prop.relation.cardinality, new Set(["one-to-one", "one-to-many", "many-to-many"]), `${here}.relation.cardinality`, limits); if (prop.relation.maxItems !== undefined && (!Number.isSafeInteger(prop.relation.maxItems) || prop.relation.maxItems < 1)) fail(`${here}.relation.maxItems must be a positive safe integer`); if (prop.relation.onDelete !== undefined) oneOf(prop.relation.onDelete, new Set(["retain", "remove"]), `${here}.relation.onDelete`, limits); } if (prop.relationDatabaseId !== undefined) stableId(prop.relationDatabaseId, `${here}.relationDatabaseId`, limits); }
    const liveProperties = db.properties.filter(property => property.deletedAt === undefined), livePropertyIds = new Set(liveProperties.map(property => property.id)), livePropertyMap = new Map(liveProperties.map(property => [property.id, property] as const));
    if (db.propertyOrder !== undefined && !Array.isArray(db.propertyOrder)) fail(`${path}.propertyOrder must be an array`);
    const propertyOrder = (db.propertyOrder ?? liveProperties.map(property => property.id)) as ID[];
    const orderedPropertyIds = propertyOrder.map((value, orderIndex) => stableId(value, `${path}.propertyOrder[${orderIndex}]`, limits));
    if (new Set(orderedPropertyIds).size !== orderedPropertyIds.length || orderedPropertyIds.length !== liveProperties.length || orderedPropertyIds.some(propertyId => !livePropertyIds.has(propertyId))) fail(`${path}.propertyOrder must contain every live property exactly once`);
    const titleProperties = db.properties.filter(property => property.type === "title");
    if (titleProperties.length && (titleProperties.length !== 1 || titleProperties[0]!.deletedAt !== undefined)) fail(`${path} must retain exactly one live title property`);
    if (db.titlePropertyId !== undefined && (titleProperties.length !== 1 || stableId(db.titlePropertyId, `${path}.titlePropertyId`, limits) !== titleProperties[0]!.id)) fail(`${path}.titlePropertyId must identify the canonical live title property`);
    if (titleProperties.length && !orderedPropertyIds.some(propertyId => livePropertyMap.get(propertyId)?.type === "title")) fail(`${path}.propertyOrder must retain the title property`);
    ...
    for (const [v, view] of db.views.entries()) { const here = `${path}.views[${v}]`; if (!plain(view)) fail(`${here} must be a plain object`); const viewId = unique(view.id, `${here}.id`, limits, allIds); viewIds.add(viewId); if (view.collectionId !== undefined && stableId(view.collectionId, `${here}.collectionId`, limits) !== id) fail(`${here} references another collection`); string(view.name, `${here}.name`, limits, true); oneOf(view.type, VIEW_TYPES, `${here}.type`, limits); if (!Array.isArray(view.visiblePropertyIds)) fail(`${here}.visiblePropertyIds must be an array`); const propertyRefs = [view.visiblePropertyIds, view.propertyOrder ?? []]; for (const refs of propertyRefs) { if (!Array.isArray(refs)) fail(`${here} property IDs must be arrays`); for (const ref of refs) if (!livePropertyIds.has(stableId(ref, `${here} property ID`, limits))) fail(`${here} references tombstoned or unknown property ${ref}`); } if (view.columnWidths !== undefined) { if (!plain(view.columnWidths)) fail(`${here}.columnWidths must be a plain object`); for (const [propertyId, width] of Object.entries(view.columnWidths)) { stableId(propertyId, `${here}.columnWidths key`, limits); if (!livePropertyIds.has(propertyId)) fail(`${here}.columnWidths references tombstoned or unknown property ${propertyId}`); if (typeof width !== "number" || !Number.isFinite(width) || width <= 0) fail(`${here}.columnWidths.${propertyId} must be positive`); } } if (view.filters !== undefined) validateFilter(view.filters, `${here}.filters`, limits, livePropertyMap, attachmentIds); if (view.sorts !== undefined) { if (!Array.isArray(view.sorts)) fail(`${here}.sorts must be an array`); for (const [s, sort] of view.sorts.entries()) { if (!plain(sort)) fail(`${here}.sorts[${s}] must be a plain object`); const propertyId = stableId(sort.propertyId, `${here}.sorts[${s}].propertyId`, limits); if (!livePropertyIds.has(propertyId)) fail(`${here}.sorts[${s}] references tombstoned or unknown property`); oneOf(sort.direction, new Set(["asc", "desc"]), `${here}.sorts[${s}].direction`, limits); if (sort.nulls !== undefined) oneOf(sort.nulls, new Set(["first", "last"]), `${here}.sorts[${s}].nulls`, limits); if (sort.locale !== undefined) string(sort.locale, `${here}.sorts[${s}].locale`, limits); } } for (const field of ["groupByPropertyId", "subgroupByPropertyId", "calendarDatePropertyId", "timelineStartPropertyId", "timelineEndPropertyId"] as const) if (view[field] !== undefined && !livePropertyIds.has(stableId(view[field], `${here}.${field}`, limits))) fail(`${here}.${field} references tombstoned or unknown property`); for (const field of ["layout", "cardPreview", "permissions"] as const) if (view[field] !== undefined) safeObject(view[field], `${here}.${field}`, limits); if (view.scope !== undefined) oneOf(view.scope, new Set(["personal", "shared"]), `${here}.scope`, limits); }
    for (const [recordIndex, value] of (db.recordPageIds ?? []).entries()) {
      ...
      const recordPage = pageById.get(recordPageId)!;
      if (recordPageOwners.has(recordPageId)) fail(`${recordPath} duplicates record page membership across one or multiple databases`);
      if (recordPage.collectionId === undefined) fail(`${recordPath} lists page ${recordPageId} that is not a record`);
      if (recordPage.collectionId !== id) fail(`${recordPath} lists a record page from another collection`);
      recordPageOwners.set(recordPageId, id);
      for (const property of liveProperties) if (property.type !== "title" && property.validation?.required && recordPage.properties?.[property.id] == null) fail(`${recordPath} is missing required property ${property.id}`);
    }
  }
  ...
  for (const [pageIndex, page] of w.pages.entries()) {
    ...
    if (recordPageOwners.get(page.id) !== collection.id) fail(`pages[${pageIndex}] record page is not indexed by its collection record membership`);
