...
function validatePropertyValue(value: unknown, property: DatabaseProperty, path: string, limits: ValidationLimits, attachmentIds: ReadonlySet<ID>): void {
  ...
  if (validation !== undefined) {
    ...
    if (typeof value === "string") {
      ...
      if (validation.pattern !== undefined && !new RegExp(validation.pattern, "u").test(value)) fail(`${path} does not match validation pattern`);
    }
  }
}
...
function validatePropertyMetadata(property: DatabaseProperty, path: string, limits: ValidationLimits): void {
  ...
  if (!plain(property.validation) || Object.keys(property.validation).some(key => !["required", "min", "max", "minLength", "maxLength", "pattern"].includes(key))) fail(`${path}.validation has an invalid shape`);
  ...
  if (validation.pattern !== undefined) { const pattern = string(validation.pattern, `${path}.validation.pattern`, limits, true); try { new RegExp(pattern, "u"); } catch { fail(`${path}.validation.pattern must be a valid regular expression`); } }
}
...
export function assertWorkspaceValue(value: unknown, overrides: Partial<ValidationLimits> = {}): asserts value is Workspace {
  ...
  for (const [index, item] of w.databases.entries()) {
    ...
    const liveProperties = db.properties.filter(property => property.deletedAt === undefined);
    ...
    if (liveProperties.some(property => property.type === "title") && !orderedPropertyIds.some(propertyId => properties.get(propertyId)?.type === "title")) fail(`${path}.propertyOrder must retain the title property`);
